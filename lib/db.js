...
const EMPTY = {
  ...
  sections: [],
  schools: [],
  unlocks: [], // [{sectionId, unitId, unlockedBy, unlockedAt}]
  attempts: [], // [{userId, unitId, exerciseId, correct, at}]
};
...
export function setSectionTeachers(id, teacherIds) {
  ...
  store.sections[idx] = {
    ...store.sections[idx],
    teacherIds: Array.from(new Set((teacherIds || []).filter(Boolean))),
  };
  writeStore(store);
  return store.sections[idx];
}

// ---------- chapter unlocks (per section) ----------

export function listUnlocks(filter = {}) {
  const store = readStore();
  return (store.unlocks || []).filter((u) =>
    Object.entries(filter).every(([k, v]) =>
      v === undefined ? true : u[k] === v,
    ),
  );
...
export function isUnitUnlocked(sectionId, unitId) {
  ...
  return (readStore().unlocks || []).some(
    (u) => u.sectionId === sectionId && u.unitId === unitId,
  );
}

export function setUnitUnlocked(sectionId, unitId, unlockedBy, unlocked) {
  const store = readStore();
  store.unlocks = store.unlocks || [];
  const idx = store.unlocks.findIndex(
    (u) => u.sectionId === sectionId && u.unitId === unitId,
  );
  if (unlocked) {
    if (idx < 0)
...
export function getAttemptsForUser(userId) {
  return (readStore().attempts || []).filter((a) => a.userId === userId);
}

export function getAttemptsForUnit(userId, unitId) {
  return (readStore().attempts || []).filter(
    (a) => a.userId === userId && a.unitId === unitId,
  );
}
